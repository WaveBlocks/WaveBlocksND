...
the program ``hdfview`` for example to gain some insight of the contents of the file.
More tools for manipulating `hdf` files can be found on the webpage of the [hdfgroup]_.

The ``Main.py`` command (like all other commands) supports an online help
listing the available options and switches:
