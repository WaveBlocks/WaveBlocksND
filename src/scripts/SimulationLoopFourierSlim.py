...
__all__ = ["SimulationLoopFourierSlim"]


class SimulationLoopFourierSlim(SimulationLoop):
    ...
    def prepare_simulation(self):
        ...
        self._opV = potential.evaluate_at(grid, as_matrix=True)

        # Prepare another empty wavefunction class
        self._WF2 = WaveFunction(self.parameters)
        self._WF2.set_grid(grid)
        self._WF3 = WaveFunction(self.parameters)
        self._WF3.set_grid(grid)
        ...
        # Make the eigentransform w/o destroying the original
        WF = self.propagator.get_wavefunction()
        self._WF2.set_values(WF.get_values())
        self._BT.transform_to_eigen(self._WF2)

        # Save the norm at the beginning of our simulation
        norms = self._WF2.norm()
        ...
        for index, item in enumerate(values):
            ...
            tmp[index] = item
            self._WF3.set_values(tmp)

            # Project this vector to the canonical basis
            self._BT.transform_to_canonical(self._WF3)

            # And calculate the energies of these components
            ekinlist.append(self._WF3.kinetic_energy(self._opT, summed=True))
            epotlist.append(self._WF3.potential_energy(self._opV, summed=True))
