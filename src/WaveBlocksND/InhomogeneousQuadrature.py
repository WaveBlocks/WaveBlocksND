...
class InhomogeneousQuadrature(Quadrature):
    ...
    def quadrature(self, pacbra, packet, operator=None, summed=False, component=None, diag_component=None):
        ...
        Kbra = [ bs.get_basissize() for bs in pacbra.get_basis_shape() ]
        Kket = [ bs.get_basissize() for bs in packet.get_basis_shape() ]
    ...
    def build_matrix(self, pacbra, packet, operator=None):
        ...
        # Operator is None is interpreted as identity transformation
        if operator is None:
            operator = lambda void, nodes, component=None: ones(nodes.shape[1]) if component[0] == component[1] else zeros(nodes.shape[1])
