...
def add_wavepacket(self, parameters, timeslots=None, blockid=0, key=("q","p","Q","P","S")):
    ...
    daset_tg = grp_wp.create_dataset("timegrid", (T,), dtype=np.integer, chunks=True, maxshape=(Ts,), fillvalue=-1)
    daset_bs = grp_wp.create_dataset("basis_shape_hash", (T, N), dtype=np.integer, chunks=True, maxshape=(Ts,N))
    daset_bsi = grp_wp.create_dataset("basis_size", (T, N), dtype=np.integer, chunks=True, maxshape=(Ts,N))
    # Parameters
    if "q" in key and not "q" in grp_pi.keys():
        daset_q = grp_pi.create_dataset("q", (T,D,1), dtype=np.complexfloating, chunks=True, maxshape=(Ts,D,1))
    if "p" in key and not "p" in grp_pi.keys():
        daset_p = grp_pi.create_dataset("p", (T,D,1), dtype=np.complexfloating, chunks=True, maxshape=(Ts,D,1))
    if "Q" in key and not "Q" in grp_pi.keys():
        daset_Q = grp_pi.create_dataset("Q", (T,D,D), dtype=np.complexfloating, chunks=True, maxshape=(Ts,D,D))
    if "P" in key and not "P" in grp_pi.keys():
        daset_P = grp_pi.create_dataset("P", (T,D,D), dtype=np.complexfloating, chunks=True, maxshape=(Ts,D,D))
    if "S" in key and not "S" in grp_pi.keys():
        daset_S = grp_pi.create_dataset("S", (T,1,1), dtype=np.complexfloating, chunks=True, maxshape=(Ts,1,1))
    if "adQ" in key and not "adQ" in grp_pi.keys():
        daset_adQ = grp_pi.create_dataset("adQ", (T,1,1), dtype=np.complexfloating, chunks=True, maxshape=(Ts,1,1))
    # Coefficients
    for i in xrange(N):
